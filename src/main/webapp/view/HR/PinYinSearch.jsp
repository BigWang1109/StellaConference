...
//        var flag = chrStr.charCodeAt(0) <= 255;
//        alert(flag);
...
//          return tmpchr;
      ...
      convertPYNew: function(chrStr) {
        if (chrStr == null || chrStr.length == 0)
          return "";
        var tmpchr = chrStr.charAt(0);
        if (chrStr.charCodeAt(0) <= 255)
          return tmpchr;
        for (var i = 0; i < this._pyvalue.length; i++) {
          if (this._pyvalue[i].indexOf(tmpchr) >= 0)
            return this._pystr[i];
        }
        return '';
      },
      ...
      convertPYsNew: function(str) {
        var arr = str.split('');
        var arrPY = [],
                arrPYS = [];
        var ssht;
        for (var i = 0; i < arr.length; i++) {
          ssht = this.convertPYNew(arr[i]);
          if (ssht) {
            arrPY.push(ssht);
            arrPYS.push(ssht.charAt(0));
          }
    ...
    function query() {
      var str = getEl('txtChinese').value.trim();
      if (!str) {
        return '';
      }
      ...
      getEl('divResult').innerHTML = arr.join(' | ');
    }
    function queryNew() {
      var str = getEl('txtPinyin').value.trim();
      if (!str) {
        return '';
      }

      var arr = SimplePinYin.convertPYsNew(str);
...
<div class='wrapper'>
  <div>
    <span>请输入中文字符串:</span><input type="text" id="txtChinese" size="30" value="芈月传">
    <input type="button" onclick="query()" value="查看拼音">
  </div>
  <div>
    <span>请输入首字母:</span><input type="text" id="txtPinyin" size="30" value="wxx">
    <input type="button" onclick="queryNew()" value="查看汉字">
  </div>
