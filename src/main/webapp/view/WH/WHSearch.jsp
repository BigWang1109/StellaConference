    ...
    function createInvigilatorNew(data){
      ...
      if(data.length ==0){
        ...
        for(var i=0;i<data.length;i++){
          ...
          '<td class="psnname"><a href="<%=path%>/WH/userSearchByCode/'+data[i].PSNCODE+'">'+data[i].PSNNAME+'</a></td>' +
          '<td class="corpname"><a href="<%=path%>/WH/userSearchByCode/'+data[i].PSNCODE+'">'+data[i].CORPNAME+'</a></td>' +
          '</tr>');
        }
      }
    }
    function userSearchById(PSNCODE){
      window.location.href = '<%=path%>/WH/userSearchByCodeNew/'+PSNCODE;
    }
    function cusSearch(){
      ...
      if(checkValue == ''){
        ...
        $.ajax({
          url : '<%=path%>/WH/CVSearchByPage/'+checkValue+"/"+myPageIndex,
