...
////        isAndroid = u.indexOf('Android') > -1 || u.indexOf('Linux') > -1;
        ...
        var now = getNowTime();
        waterMark({"watermark_txt":userId+"-"+userName+"-"+now})
////        alert(isAndroid);
      });
  ...
  <div id="print">
    <table id="cvtable" border="1px">
      ...
      <tr>
        ...
        <td class="cvvalue" rowspan="5"><img id="picture" src="<%=path%>/YTCX/getPictureByCode/${psndoc.PSNCODE}"  width="345" style="width: 85px;height: 115px;"/></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td class="cvname">人员类别</td><td class="cvvalue" >${psndoc.psnclassname}</td>
      </tr>
      <tr>
      ...
      <tr>
        <td class="cvname">家庭住址</td><td class="cvvalue" colspan="3">${psndoc.ADDR}</td>
      </tr>
      ...
      <c:forEach items="${workList}" var="work">
        <tr class="workItem">
          <td class="cvvalue">${work.BEGINDATE}<br>${work.ENDDATE}</td>
          <td class="cvvalue">${work.WORKCORP}</td>
          <td class="cvvalue">${work.WORKDEPT}</td>
      ...
      <tr><td id="famBar" class="cvtitle"  colspan="4" onclick="hideAndShow('famItem','合同信息','famBar')">家庭信息</td></tr>
      <c:if test="${famList != null && fn:length(famList) > 0}">
        <tr class="famItem">
          <td class="cvname">关系/姓名</td>
          <td class="cvname">出生日期</td>
          <td class="cvname">工作单位</td>
          <td class="cvname">联系电话</td>
        </tr>
      </c:if>
      <c:forEach items="${famList}" var="fam">
        <tr class="famItem">
          <td class="cvvalue">${fam.MEM_RELATION}</td>
          <td class="cvvalue">${fam.MEM_BIRTHDAY}</td>
          <td class="cvvalue">${fam.MEM_CORP}</td>
          <td class="cvvalue">${fam.RELAPHONE}</td>
        </tr>
      </c:forEach>
      ...
      <tr class="fileitem">
        ...
        <td class="cvvalue" colspan="4">
          ...
          <a id="${file.PK_ATTACHMENT}" href="/pdfjs/web/viewer.html?file=<%=path%>/YTCX/downloadFileFormat/${file.PK_ATTACHMENT}/${file.PSNCODE}" onclick="viewBeforeCheck('${file.ATTACHMENT_NAME}','${file.PK_ATTACHMENT}')">${file.ATTACHMENT_NAME}</a>
          </c:if>
            <c:if test="${isAndroid == true}">
              <a id="${file.PK_ATTACHMENT}" href="<%=path%>/YTCX/turnToPdfView/${file.PK_ATTACHMENT}/${file.PSNCODE}" onclick="viewBeforeCheck('${file.ATTACHMENT_NAME}','${file.PK_ATTACHMENT}')">${file.ATTACHMENT_NAME}</a>
            </c:if>
        </td>
      </tr>
      </c:forEach>
